...
Changes of XMind 8 (v3.7.0)

New Features

1. New Slide-based Presentation.
2. New Workspace UI with better efficiency for map editing.
3. Improved XMind Cloud: create folders and sort files in Cloud.
4. New useful Templates.
5. Brand new Clip Art.
6. Import & Export to OPML, Lighten file.
7. Resource Manager. Organize all resource in one place, including Clip Art, Marker, Theme, Style and Template.


Major Improvements

1. High resolution display support.
2. Import Novamind maps.
3. Allow Manual Layout.
4. Support adding idea groups in Brainstorming mode.
5. Redesigned file encryption function and support password hint.
6. New Audio Notes operation interface.
7. Optimize export function and list. Add recent used list.
8. "Insert image from Iconfinder" interaction optimization.
9. Reorganized context menu.
10. Preferences interface optimization.
11. "Numbering" usability enhancement.
12. Keyboard shortcuts presentation operation optimization in Walkthrough Mode.
13. Enhanced template management UI.
14. Many other minor improvements.


Bug Fixed

1. Fix an issue that when there still are cloud files not saved and sign out the XMind account, then it will cause error when click save button.
2. Fix an issue that causes sync error as the map attachment can not be uploaded successfully.
3. Fix an issue that might cause maps can not be uploaded to XMind Cloud.
4. Fix an issue that the cloud files should not be able to be opened if the synchronization is not finished.
5. Fix an issue that causes Cloud icon to disappear on program initialization interface.
6. Fix an issue that causes comments to be lost after setting a password to the map.
7. Fix an issue that causes Audio Notes unable to use after setting a password to the map.
8. Fix an issue that causes the exported SVG file incorrect display.
9. Fix an issue that causes the exported SVG file incorrect display if the map was exported under Drill Down mode.
10. Fix an issue that might cause markers to be lost sometimes after saving a map.
11. Fix an issue that causes error when save as a map, which is opened in XMind but has been deleted from local.
12. Fix an issue that cause error when using keyboard to copy and paste on a relationship.
13. Fix an issue that after reducing file size, deleting the preview image and save as a new map, then the new map will lose the comments.
14. Fix a bug that after modifying a comment, the comment tips still show as previous version.
15. Fix an issue that causes the skeleton to disappear after applying some themes onto a map which is under fishbone structure.


Changes of XMind 7 (v3.6.50)

New Features

1. XMind Cloud
2. New Share Dialog
3. 60,000+ Pretty Icons from Iconfinder.com

Major Improvements

1. Brand New Vector Markers (SVG)
2. Colorful sheet tabs
3. More "Multi Branch Colors" options
4. Copy/Paste Topics optimization
5. Rename sheet interaction optimization
6. Map Shot interaction optimization
7. Group themes
8. Add "All" option in droplist menu of Theme Editor dialog
9. Print Dialog optimization
10. New Notification UI
11. Add checkbox option to dictionaries in Spelling settings
12. Many other improvements and minor bug fixes


Changes of XMind 7 (v3.6.1)

Improvements

1. Modify the translation errors in French, German, Korean language pack.
2. Update Help File in Chinese version.

Bug Fixes

1. Fix an issue that causes error while activating XMind 7 Pro license.
2. Fix an issue that "Extend" and "Extend All" functions do not work properly.
3. Fix an issue that causes comment to miss after moving the topic.
4. Fix an issue that the orders of columns and rows can not be moved under Matrix structure.
5. Fix an issue that causes error while inserting an image from web.
6. Fix an issue that the list of recent files updated incorrectly.
7. Fix a bug that causes exported PDF Map incorrect display even user selected "Show +/-" in the export to PDF dialog.
8. Fix an issue when the central topic of a map has no text, the map can not be uploaded to xmind.net.
9. Fix an issue that in Chinese, Japanese and Korean version, exported Excel file is incorrect when choosing Pivot and Outline layout.
10. Fixed a bug when inserting too many sheets in a workbook, some of the sheets lost from the Sheet Tab.
11. Fix an issue that causes failure to import Mindmanager file
12. Fix an issue that causes failure to export to Freemind.
13. Fix an issue that failure to open a file after removing this XMind file's password.
14. Fix a bug that causes the color group not to display correctly in the font dialog.
15. Other minor bug fixes.
